...
if ($member->title) {
    // Si c'est un tableau (Spatie Translatable)
        if (is_array($member->title->name)) {
            $display_role =
                $member->title->name[$lang] ?? ($member->title->name['fr'] ?? array_values($member->title->name)[0]);
        }
                ...
                <div class="card border-0 shadow-lg text-center p-4 sticky-top" style="top: 100px; z-index: 10;">
                    ...
                    <div class="mb-4">
                        ...
                        @if (!empty($member->city))
                            <div class="text-muted small mt-1"><i class="fas fa-map-marker-alt me-1"></i>
                                {{ $member->city }}</div>
                        @endif
                    </div>
                    {{-- FIN AJOUT --}}
                    ...
                    <div class="d-grid gap-2 text-start px-2">
                        ...
                        @if (!empty($member->advisor_code))
                            <div class="mt-4 p-3 rounded"
                                style="background-color: #f8f1e5; border: 1px dashed #c9a050;">
                                ...
                                <a href="{{ route('consent.show', ['locale' => app()->getLocale(), 'code' => $member->advisor_code]) }}"
                                    class="btn btn-dark w-100 fw-bold shadow-sm py-2">
                                    <i class="fas fa-car me-2 text-warning"></i>
                                    {{ __('team_detail.auto_insurance_btn') }}
                                </a>
                            </div>
                        @endif
                    </div>
