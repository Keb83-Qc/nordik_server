<x-filament-panels::page>
    <div class="grid grid-cols-1 gap-4">
        <div class="overflow-x-auto bg-white rounded-xl shadow-sm border border-gray-200 dark:bg-gray-900 dark:border-gray-800">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
                <div class="flex items-center gap-4">
                    ...
                    <span class="text-xs text-gray-500">
                        {{ count($selectedAdvisors) }} sélectionné(s)
                    </span>
                </div>

                <button
                    wire:click="sendBulkLinks"
                    wire:loading.attr="disabled"
                    class="text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-xs px-3 py-2 transition-all duration-200">
                    <span wire:loading.remove wire:target="sendBulkLinks">
                        Envoyer en masse
                    </span>
                    <span wire:loading wire:target="sendBulkLinks">
                        Envoi...
                    </span>
                </button>
            </div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    $link = route('consent.show', ['locale' => $locale, 'code' => $advisor->advisor_code]);
                    @endphp
                    ...
                    <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                        ...
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white flex items-center gap-3">
                            @if($advisor->image)
                            <img src="{{ $advisor->image_url }}" class="w-8 h-8 rounded-full object-cover">
                            @else
                        ...
                        <td class="px-6 py-4 text-center w-48">
                            ...
                            <div>
                                <button
                                    wire:click="sendLink({{ $advisor->id }})"
                                    wire:loading.attr="disabled"
